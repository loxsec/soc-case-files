...
| 5 *(if attacker hypothesis survives)* | Attacker did something with access after the 12:36:31 success vs. session was benign | Pull all post-12:36:31 activity: commands, sudo attempts, new SSH keys, outbound connections | Post-login session data (commands run, sudo attempts, new SSH keys, outbound connections) is not available for this lab. The original logging setup only captured /var/log/auth.log and /var/log/sshd.log at the authentication layer — no auditd/execve tracking or session-command logging was configured at the time of the attack, and that data was not retained after the fact. This investigation is therefore scoped to authentication-layer analysis: confirming the compromise occurred and characterizing how, not what happened afterward. Full post-login visibility requires host-level telemetry (auditd, EDR, or a SIEM agent monitoring live), which is exactly the gap the upcoming Wazuh project is built to close. |
...
**Next steps:** reset the compromised account's password and review why it fell in two tries; consider rate-limiting or lockout after repeated invalid-username attempts, since 24 failed guesses against a fake account produced no alert of its own. Note: this investigation was limited to authentication-layer logs; no session-level telemetry was captured for the post-login period, which is a known blind spot in this lab's setup.
